...
//	   	kprintf("not switch\n");
	...
	for(j = 0; j < NFRAMES; j ++)
  	{
    	if((frm_tab[j].fr_status == FRM_MAPPED) && (frm_tab[j].fr_type == FR_PAGE) && frm_tab[i].fr_dirty == 1)
     	{
      		int store, pageth;
      		if( frm_tab[j].fr_pid == oldpid && bsm_lookup(frm_tab[j].fr_pid, frm_tab[j].fr_vpno * NBPG, &store, &pageth) != SYSERR) 
        	{
        		write_bs((j + FRAME0)*NBPG, store, pageth);
        	}
  	...
  	for(j = 0; j < NFRAMES; j++)
  	{
     	...
     	{
      		int store, pageth;
      		if( frm_tab[j].fr_pid == currpid && bsm_lookup(frm_tab[j].fr_pid, frm_tab[j].fr_vpno * NBPG, &store, &pageth) != SYSERR) 
        	{        
        		read_bs((j + FRAME0)*NBPG, store, pageth);
        	}
